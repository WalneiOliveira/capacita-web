...
export class AuthService {
  ...
  // MÉTODO NOVO: Realiza a chamada HTTP para alterar a senha
  alterarSenha(senhaAtual: string, novaSenha: string): Observable<{ user: User }> {
    return this.http
      .post<{ user: User }>(`${this.apiUrl}/alterar-senha`, { senhaAtual, novaSenha })
      .pipe(
        tap((resposta) => {
          // Atualiza o usuário no estado e no sessionStorage com precisaTrocarSenha = false
          this.atualizarUsuarioNaSessao(resposta.user);
        })
      );
  }
  ...
  // MÉTODO AUXILIAR: Atualiza apenas as informações do usuário logado na sessão
  private atualizarUsuarioNaSessao(usuario: User): void {
